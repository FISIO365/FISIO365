...
Redacta el informe con EXACTAMENTE estas secciones en este orden:
...
NO incluyas PLAN DE TRATAMIENTO, RECOMENDACIONES, PRONÓSTICO ni otras secciones.`;

      // Solo los datos no vacíos para reducir el prompt
      const compactD = Object.entries(d)
        .filter(([k,v]) => v && v !== '-' && v !== '--' && !(Array.isArray(v) && !v.length))
        .map(([k,v]) => `${k}: ${Array.isArray(v) ? v.join(', ') : v}`)
        .join('\n');

      const prompt = `Eres fisioterapeuta experto. Redacta informe de valoración fisioterapéutica de ${tipo.toUpperCase()}. Sin markdown, en párrafos, términos técnicos explicados entre paréntesis.
${cabecera}
DATOS CLÍNICOS:
${compactD}
