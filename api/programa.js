...
module.exports = async function handler(req, res) {
  ...
  // ── GET — cargar programa existente de un paciente ────────────────────────
  if (req.method === 'GET') {
    ...
    try {
      const url = `https://api.airtable.com/v0/${BASE_ID}/${PLAN_TABLE}?filterByFormula={PacienteID}="${pacienteId}"&fields[]=Ejercicios&fields[]=MensajeFisio&fields[]=FechaAsignacion&sort[0][field]=FechaAsignacion&sort[0][direction]=desc&pageSize=1`;
  ...
  // ── POST — guardar/reemplazar programa ────────────────────────────────────
