...
export default async function handler(req) {
  ...
  if (action === 'listar-informes') {
    ...
    try {
      ...
      const [r1, r2] = await Promise.all([
        ...
        fetch(`https://api.airtable.com/v0/${BASE_ID}/${INFORMES_TABLE_ID}?sort[0][field]=Fecha&sort[0][direction]=desc&pageSize=50`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } })
      ]);
  ...
  if (action === 'responder-mensaje' && req.method === 'POST') {
    ...
    try {
      ...
      await crearNotifEnServer(AIRTABLE_TOKEN, BASE_ID, pacienteId, `Respuesta de ${fisioNombre||'tu fisio'}`, respuesta||'', 'respuesta');
